...
from .mixins import (
    AdminVendorCustomerOnlyMixin, AdminOnlyMixin, VendorFilterMixin,
    CustomerFilterMixin, UserRestrictedMixin, CustomerReadOnlyMixin,
...
class VendorProfileAdmin(AdminVendorCustomerOnlyMixin, admin.ModelAdmin):
    list_display = ("user_email", "vendor_business_name", "vendor_is_verified", "vendor_created_at")
    ...
    fieldsets = (
        ...
        ('Verification', {
            'fields': ('is_verified', 'verification_documents') if hasattr(VendorProfile, 'is_verified') else ('verification_documents',)
        }),
    )
...
class ProductAdmin(AdminVendorCustomerOnlyMixin, VendorFilterMixin, VendorAutoAssignMixin, admin.ModelAdmin):
    list_display = ("name", "vendor_name", "price", "stock_quantity", "is_active", "created_at")
    ...
    fieldsets = (
        ...
        ('SEO & Tags', {
            'fields': ('meta_description', 'tags') if hasattr(Product, 'tags') else ('meta_description',),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
...
class OrderAdmin(AdminVendorCustomerOnlyMixin, CustomerFilterMixin, CustomerReadOnlyMixin, admin.ModelAdmin):
    list_display = ("id", "user_email", "status", "payment_status", "order_total_amount", "created_at")
    ...
    fieldsets = (
        ...
        ('Financial', {
            'fields': ('total_amount', 'shipping_cost', 'tax_amount', 'discount_amount')
        }),
        ('Addresses', {
...
class WishlistAdmin(AdminVendorCustomerOnlyMixin, CustomerFilterMixin, admin.ModelAdmin):
...
class ShippingAddressAdmin(AdminVendorCustomerOnlyMixin, CustomerFilterMixin, admin.ModelAdmin):
    list_display = ("user_email", "full_name", "shipping_address_line_1", "city", "shipping_country", "is_default")
...
class PaymentAdmin(admin.ModelAdmin):
    list_display = ("id", "contract_title", "amount", "payment_status_display", "payment_type_display", "payment_created_at")
    search_fields = ("contract__title", "transaction_id")
    readonly_fields = ("transaction_id",)

    fieldsets = (
        ('Payment Info', {
            'fields': ('contract', 'amount', 'payment_type', 'status')
        }),
        ('Transaction', {
    ...
    def contract_title(self, obj):
        return obj.contract.title if hasattr(obj, 'contract') and obj.contract else "N/A"
